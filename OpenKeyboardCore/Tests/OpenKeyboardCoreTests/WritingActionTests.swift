...
final class WritingActionTests: XCTestCase {
    func testSharedContractVersionIsPinned() {
        XCTAssertEqual(WritingPromptBuilder.contractVersion, "4.0.0")
    }
    ...
    func testEveryBuiltInPromptContainsStrictContractAndOperationRules() throws {
        ...
        for scenario in scenarios {
            ...
            if scenario.action == .rewrite {
                ...
                XCTAssertEqual(rendering.messages.count, 2)
                let instructions = rendering.messages.first?.content ?? ""
                for rule in scenario.requiredRules {
                    XCTAssertTrue(instructions.localizedCaseInsensitiveContains(rule), "\(scenario.operation) missing rule: \(rule)")
                }
                continue
            }
