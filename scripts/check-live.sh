...
if [[ "$LIVE_MODE" == "gateway-differential" ]]; then
  ...
  SUMMARIZE_LINE="$(grep -E '^summarize_outcomes=' "$evidence_file")"
  CONTINUE_WRITING_LINE="$(grep -E '^continue_writing_outcomes=' "$evidence_file")"
  WARNING_LINE="$(grep -E '^operation_scoped_warning_contracts=' "$evidence_file")"
  LATENCY_LINE="$(grep -E '^profile_latencies=' "$evidence_file")"
  DIAGNOSTIC_OUTCOMES_LOW_LINE="$(grep -E '^diagnostic_outcomes_low=' "$evidence_file")"
  DIAGNOSTIC_LATENCIES_LOW_LINE="$(grep -E '^diagnostic_latencies_low=' "$evidence_file")"
  DIAGNOSTIC_OUTCOMES_HIGH_LINE="$(grep -E '^diagnostic_outcomes_high=' "$evidence_file")"
  DIAGNOSTIC_LATENCIES_HIGH_LINE="$(grep -E '^diagnostic_latencies_high=' "$evidence_file")"
  ...
  [[ "$DIAGNOSTIC_LATENCIES_LOW_LINE" =~ ^diagnostic_latencies_low=transport=[0-9]+ms,\ grammar=[0-9]+ms,\ rewrite=[0-9]+ms,\ translation=[0-9]+ms$ ]] || fail "Low-profile diagnostic capability latencies are malformed."
  ...
  [[ "$DIAGNOSTIC_LATENCIES_HIGH_LINE" =~ ^diagnostic_latencies_high=transport=[0-9]+ms,\ grammar=[0-9]+ms,\ rewrite=[0-9]+ms,\ translation=[0-9]+ms$ ]] || fail "High-profile diagnostic capability latencies are malformed."
else
