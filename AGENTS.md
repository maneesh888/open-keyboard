...
Remote GitHub CI runs hygiene, `core`, semantic-contract validation, and `build` from `.github/workflows/ci.yml`, then reports `Required technical checks`. Complete exact-head requirement and independent-review evidence separately emits `Required checks`; initial incomplete evidence emits only the non-required `Incomplete review evidence` failure. Review metadata events are serialized per PR and each immutable event snapshot is validated, so a rapid invalid edit and restoration cannot collapse into one valid state. The PR must link the newest same-head project-reviewer COMMENTED report. Once `Required checks` has succeeded for a SHA, any later invalid/uninspectable evidence or protected failure emits/retains a failed `Required checks` and permanently requires a new commit plus a complete exact-head proof cycle. `.github/workflows/live.yml` enforces retained exact-head local evidence for gateway-impacting changes without receiving credentials. Do not imply remote CI proves simulator UI, screenshots, real keyboard extension behavior, live gateway execution, signing, or deployment.
...
- Before a guarded merge, always re-fetch and validate the current body, linked review, head, threads, and complete check history; require a durable linked independent-review report, the exact reviewed head to pass `./scripts/check.sh --full`, `Required technical checks`, `Required checks`, `Required live verification`, applicable live evidence, and unresolved-thread checks. A prior failed `Required checks` permanently burns that SHA. Automatic authorization additionally requires every in-scope requirement `VERIFIED`, no material uncertainty, and exact reviewer confidence of `100%`. Otherwise keep the PR draft and require explicit repository-owner approval for that exact SHA after all unverified requirements and blockers are disclosed; never infer or carry that approval across a new head.
