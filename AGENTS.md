...
5. **Implement narrowly.** Read only the sources and focused plans needed for the task. Route a
   read-only UI audit through `$audit-openkeyboard-ui`. For authorized UI fixes, use that skill
   within `$develop-openkeyboard` to trace the target screen's production behavior and review
   adjacent-screen continuity without expanding the edit scope. Within the audit, delegate
   wording-specific analysis to `$write-openkeyboard-product-copy`. For work centered on
   user-visible app or keyboard wording, use `$write-openkeyboard-product-copy` within
   `$develop-openkeyboard` to shape the target screen and its connected journey before editing.
   Source inspection may establish code findings and visual risks, but never rendered or runtime
   acceptance. Reuse local patterns, add focused regression coverage for changed behavior, and
   preserve files outside the work order.
...
The trusted private schema is documented in `docs/LIVE_EVIDENCE_PRIVACY.md`. Load the classifier
and validator from the exact trusted base; reject missing or legacy validators. Never substitute
candidate policy, project synthetic model/timing fields, or invent measurements or outcomes.
