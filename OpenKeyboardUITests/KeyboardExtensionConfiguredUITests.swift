...
final class KeyboardExtensionConfiguredUITests: XCTestCase {
    ...
    func testRealKeyboardGrammarCorrectionSelectionWhenGatewayConfigured() throws {
        ...
        let mixed = "Our support team definitely need clearer notes before they reply to the customer about the delayed refund."
        let encodedSource = source.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? source
        let hostArguments = [
            "--keyboard-host-test",
            "--keyboard-host-autofocus",
            "--keyboard-host-prefer-openkeyboard",
        ...
        let input = app.textViews["keyboard_host_text_editor"]
        XCTAssertTrue(input.waitForExistence(timeout: 10), "Host app text editor was not available for live grammar verification")
        XCTAssertEqual(input.value as? String, source)
        tapCenter(of: input)

        let springboard = XCUIApplication(bundleIdentifier: "com.apple.springboard")
        let keyboardApp = XCUIApplication()
        guard waitForOpenKeyboard(keyboardApp: keyboardApp, hostInput: input, springboard: springboard) else {
            XCTFail("Open Keyboard extension did not appear")
            return
        }

        let firstRequestStartedAt = Date()
        guard waitForIssueCountBadge(keyboardApp: keyboardApp, timeout: 120) else {
            XCTFail("Live grammar request did not expose selectable corrections")
            return
        }
        let firstLatency = Date().timeIntervalSince(firstRequestStartedAt)
        print(String(format: "LIVE_GRAMMAR_REQUEST_LATENCY_SECONDS=%.3f", firstLatency))

        keyboardApp.buttons["keyboard_issue_count_badge"].tap()
        XCTAssertTrue(keyboardApp.otherElements["ai_correction_panel"].waitForExistence(timeout: 10))
        XCTAssertTrue(keyboardApp.staticTexts["keyboard_correction_progress"].label.contains("1 of 3"))
        XCTAssertEqual(keyboardApp.staticTexts["ai_correction_original"].label, "definately")
        XCTAssertEqual(keyboardApp.staticTexts["ai_correction_replacement"].label, "definitely")
        try captureRealKeyboardStep("live-grammar-three-selectable-corrections")

        keyboardApp.buttons["ai_correction_apply"].tap()
        XCTAssertTrue(keyboardApp.staticTexts["ai_correction_replacement"].waitForExistence(timeout: 5))
        XCTAssertEqual(keyboardApp.staticTexts["ai_correction_original"].label, "need")
        XCTAssertEqual(keyboardApp.staticTexts["ai_correction_replacement"].label, "needs")
        keyboardApp.buttons["ai_correction_dismiss"].tap()

        XCTAssertTrue(keyboardApp.staticTexts["ai_correction_replacement"].waitForExistence(timeout: 5))
        XCTAssertEqual(keyboardApp.staticTexts["ai_correction_original"].label, "refnd")
        XCTAssertEqual(keyboardApp.staticTexts["ai_correction_replacement"].label, "refund")
        keyboardApp.buttons["ai_correction_apply"].tap()

        XCTAssertTrue(keyboardApp.otherElements["correction_complete_panel"].waitForExistence(timeout: 10))
        XCTAssertEqual(input.value as? String, mixed)
        XCTAssertTrue(((input.value as? String) ?? "").contains("reply"), "Unrelated word 'reply' must remain unchanged")
        try captureRealKeyboardStep("live-grammar-mixed-accept-reject")

        let secondRequestStartedAt = Date()
        keyboardApp.buttons["grammar_check_again"].tap()
        XCTAssertTrue(waitForIssueCountBadge(keyboardApp: keyboardApp, timeout: 120), "Check Again did not create a fresh correction request")
        let secondLatency = Date().timeIntervalSince(secondRequestStartedAt)
        print(String(format: "LIVE_GRAMMAR_CHECK_AGAIN_LATENCY_SECONDS=%.3f", secondLatency))
        keyboardApp.buttons["keyboard_issue_count_badge"].tap()
        XCTAssertTrue(keyboardApp.otherElements["ai_correction_panel"].waitForExistence(timeout: 10))
        XCTAssertTrue(keyboardApp.staticTexts["keyboard_correction_progress"].label.contains("1 of 1"))
        XCTAssertEqual(keyboardApp.staticTexts["ai_correction_original"].label, "need")
        XCTAssertEqual(keyboardApp.staticTexts["ai_correction_replacement"].label, "needs")
        keyboardApp.buttons["ai_correction_accept_all"].tap()
        XCTAssertTrue(keyboardApp.otherElements["correction_complete_panel"].waitForExistence(timeout: 10))
        XCTAssertEqual(input.value as? String, expected)

        keyboardApp.buttons["grammar_check_again"].tap()
        XCTAssertTrue(
            keyboardApp.otherElements["correction_complete_panel"].waitForExistence(timeout: 120),
            "Clean corrected text did not complete with no further corrections"
        )
        XCTAssertFalse(keyboardApp.buttons["keyboard_issue_count_badge"].exists)
        XCTAssertEqual(input.value as? String, expected)
    }
